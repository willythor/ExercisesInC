...
2. What happens if you turn on optimization using the flag `-O2`?
    ...
    The "LFB" section gets much shorter. Many more sections start with ".L" are introduced into the code. 
...
There are less movl statements when optimized and once again there are more ".L" sections in the optimized code.    
...
Without the optimizations, the value of x is allocated into memory. Then y is calculated by adding 1 to the location of x's value in memory and that value is allocated to memory. In the case of the optimized code, 5 is never assigned to memory, but 6 (5 + 1) is automatically assigned to memory.     

What conclusions can you draw about how optimization works?

Optimizations make the assembly code quicker by decreasing the number of move statements. Decreasing move statements decrease the number of operations and results in faster assembly code. In the case of `y = x + 1` the compiler was smart enough to know that `x` wasn't used elsewhere and did the addition without evening assigning `x` to memory. This saved a couple of lines of assembly code.
